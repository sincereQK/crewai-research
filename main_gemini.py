...
# 실행할 때마다 다른 파일명 생성 (덮어쓰기 방지)
timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
output_filename = f"reports/report_{timestamp}.md"
...
search_tool = SerperDevTool()       # 구글 검색 (SERPER_API_KEY 환경변수 자동 사용)
scrape_tool = ScrapeWebsiteTool()   # 웹페이지 내용 읽기

print("✅ 검색 도구 설정 완료!")
print("   - SerperDevTool (구글 검색)")
print("   - ScrapeWebsiteTool (웹페이지 크롤링)")
print()
...
researcher = Agent(
    role="시니어 AI 리서처",
    goal="AI 에이전트 관련 최신 트렌드와 핵심 정보를 웹에서 수집한다",
    backstory="""
        당신은 10년 경력의 AI 기술 리서처입니다.
        구글 검색과 웹사이트 크롤링을 통해
        최신 기술 트렌드를 빠르게 파악하고,
        신뢰할 수 있는 소스에서 핵심 정보를 추출하는 전문가입니다.
    """,
    tools=[search_tool, scrape_tool],  # ← 검색 + 크롤링 도구!
    llm=gemini_llm,
    verbose=True,
)
...
research_task = Task(
    description="""
        2025-2026년 AI 에이전트 트렌드에 대해 웹에서 검색하여 최신 정보를 수집하세요.
        ...
        다음 내용을 포함해주세요:
        - 주요 AI 에이전트 도구들 (Claude Code, Cursor, Copilot 등)의 최신 동향
        - 멀티 에이전트 시스템의 발전 방향
        - MCP(Model Context Protocol)의 역할
        - 개발자들이 실제로 어떻게 활용하고 있는지
        
        반드시 검색 도구를 사용해서 실제 웹의 최신 정보를 기반으로 정리하세요.
        최소 10개의 핵심 포인트를 정리해주세요.
        각 포인트에 출처(URL)를 포함해주세요.
    """,
    expected_output="AI 에이전트 트렌드에 대한 상세한 리서치 결과 (최소 10개 핵심 포인트, 출처 포함)",
    agent=researcher,
)
